/*
Given an array of integers, return a new array such that each element at index i of the new array is the product of all the numbers in the original array except the one at i. Solve it without using division and in O(n) time.

For example, if our input was [1, 2, 3, 4, 5], the expected output would be [120, 60, 40, 30, 24]. If our input was [3, 2, 1], the expected output would be [2, 3, 6].
*/

public class ArrayAddOtherNumbers{

	private int[] numberArray = {1, 2, 3, 4, 5};
	private int[] NumArrayTwo = new int[5];
	private int sum;

	public static void main(String[] main){
		ArrayAddOtherNumbers mainClass = new ArrayAddOtherNumbers();
	}

	public ArrayAddOtherNumbers(){
		for(int i = 0; i < numberArray.length; i++){
			sum = 0;
			for(int f = 0; f < numberArray.length; f++){
				if(i == f){continue;
				}else if(sum == 0){
					sum = numberArray[f];
				}else{
					sum = sum * numberArray[f];
				}
			}
			NumArrayTwo[i] = sum;
		}
		

		for(int i : NumArrayTwo){
			System.out.println(i);
		}
	}

}


(1/2) * 0.8 * ((1/(1.8^2)) + ( 2 * ( (1/(2.6^2)) + (1/(3.4^2)) + (1/(4.2^2)) ) ) + (1/25) )